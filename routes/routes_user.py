...
def create_list(request: Request, list: ListModel = Body(...), service: AlunoService = Depends(get_aluno_service)):
...
def listar_alunos(request: Request, page: int = Query(1, ge=1), limit: int = Query(50, ge=1, le=10000), service: AlunoService = Depends(get_aluno_service)):
    return service.listar_usuarios(page, limit)

@router.delete('/delete-aluno', response_description="Deleta aluno")
def delete_aluno(id: str, request: Request, service: AlunoService = Depends(get_aluno_service)):
...
def update_item(id: str, request: Request, aluno_update: UpdateModel = Body(...), service: AlunoService = Depends(get_aluno_service)):
...
def buscar_aluno_cpf(cpf: str, request: Request, service: AlunoService = Depends(get_aluno_service)):
